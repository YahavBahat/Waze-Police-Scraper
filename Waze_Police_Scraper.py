...
It'll scrape all the police's locations from your preferred location,\n
by moving the mouse to that location. It also scrapes police traps.\n
Every cop that's scraped also has its own dataset. It has all the geographic location,\n
but also the number of up votes on its existence by Waze's users, confidence, and reliability, by Waze itself.\n
...
def map(key_counts_with_subtype, data_list):
    ...
    m = folium.Map(  # Map configuration
        location=[location_y_start, location_x_start],
        ...
        if sec.isdigit():  # Could have contained all than in a function
            ...
            while not err:
                try:
                    ...
                    f = open("data - its not the json data.txt", "w", encoding="utf-8")
